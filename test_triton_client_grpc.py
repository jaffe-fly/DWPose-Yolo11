"""
Test script for DWPose Triton Server deployment using gRPC protocol.

Supports both single image and video file processing with visualization.

Usage:
    # Process single image
    python test_triton_client_grpc.py --input path/to/image.jpg --url localhost:8001 --show
    
    # Process video
    python test_triton_client_grpc.py --input path/to/video.mp4 --url localhost:8001 --show --output output/result.mp4
"""
...
from ensemble_client_grpc import infer_python_backend
...
def process_image(
    ...
    print(f"Running inference on Triton Server (gRPC): {server_url}")

    # Run inference using gRPC ensemble_client
    ...
    if show:
        cv2.imshow("DWPose Triton gRPC - Image", result)
...
def process_video(
    ...
    print(f"Triton Server (gRPC): {server_url}")
    ...
    try:
        while True:
            if not paused:
                ...
                if show:
                    cv2.imshow("DWPose Triton gRPC - Video", result)
...
def main():
    parser = argparse.ArgumentParser(
        description="Test DWPose Triton Server deployment with gRPC protocol"
    )
    parser.add_argument(
        "--input", 
        type=str, 
